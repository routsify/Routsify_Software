...
const enabled = Boolean(process.env.E2E_EMAIL && process.env.E2E_PASSWORD);
...
test.skip(!enabled, "E2E credentials required");

test("Booking create update cancel with privacy consent", async ({ page }) => {
  test.setTimeout(180000);
  await login(page);
  const found = await call(page, "/api/routsify/clients?paginated=1&page=1&pageSize=50&q=info%40routsify.com");
  const clients = found.json?.data?.items || found.json?.data || [];
  ...
  try {
    const created = await call(page, "/api/routsify/clients/booking/reservations", "POST", {
      ...
      notes: `[PRUEBA E2E ROUTSIFY ${process.env.GITHUB_RUN_ID}] Reserva temporal`,
    ...
    await call(page, `/api/routsify/clients/booking/reservations/${encodeURIComponent(localId)}`, "PATCH", { notes: "[PRUEBA E2E ROUTSIFY] Actualizada" });
  } finally {
    if (localId) {
      const cancelled = await call(page, `/api/routsify/clients/booking/reservations/${encodeURIComponent(localId)}`, "DELETE", undefined, true);
      ...
      cancelledStatus = String(cancelled.json?.data?.booking?.status || cancelled.json?.data?.remote?.status || "").toLowerCase();
    }
  }
  expect(cancelledStatus).toContain("cancel");
  console.log("BOOKING_E2E", JSON.stringify({ localId, externalId, cancelledStatus }));
});
