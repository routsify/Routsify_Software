...
export function CaseWorkspace(props: WorkspaceProps & { role?: string | null }) {
  ...
  useEffect(() => {
    ...
    setLoading(tab);
    setLoadError(null);
    fetch(`/api/routsify/cases/${encodeURIComponent(props.initialCase.id)}/workspace?section=${encodeURIComponent(tab === "contrato" ? "contract" : tab === "compras" ? "purchases" : tab === "actividad" ? "activity" : tab)}`, { signal: controller.signal })
      .then(async (response) => {
  ...
  }, [tab, props.initialCase.id, canAccessTravelers, canAccessSensitive, canAccessContract, canAccessPurchases]);
  ...
  return <div className="clients-page">
    ...
    {errorPanel}
    {loadingPanel}
    {tab === "resumen" ? <SummaryTab caseRow={props.initialCase} payments={props.initialPayments || []} proposals={props.initialProposals || []} purchases={props.initialPurchases || []} onTab={(value) => setTab(value as WorkspaceTab)} /> : null}
    {tab === "viajeros" && loading !== tab ? canAccessTravelers ? <TravelersTab caseId={props.initialCase.id} initialTravelers={travelers} /> : restricted("Los datos de viajeros solo están disponibles para administración, dirección, ventas y operaciones.") : null}
    {tab === "documentos" && loading !== tab ? canAccessSensitive ? <DocumentsTab caseId={props.initialCase.id} caseCode={props.initialCase.case_code} initialDocuments={documents} travelers={travelers} /> : restricted("La documentación sensible solo está disponible para administración y agentes de ventas.") : null}
    {tab === "contrato" && loading !== tab ? canAccessContract ? <ContractPaymentsTab caseRow={props.initialCase} initialContracts={contracts} initialPayments={payments} initialFiscal={fiscal} /> : restricted("Esta sección no está disponible para tu rol.") : null}
    {tab === "compras" && loading !== tab ? canAccessPurchases ? <section className="card"><div className="section-heading"><div><h2>Compras esperadas</h2><p>Cada servicio externo debe quedar conciliado con factura o compra de Holded.</p></div><a className="btn" href={`/compras?caseId=${encodeURIComponent(props.initialCase.id)}`}>Abrir compras</a></div>{purchases.length ? <div className="table-scroll"><table><thead><tr><th>Proveedor</th><th>Servicio</th><th>Esperado</th><th>Estado</th></tr></thead><tbody>{purchases.map((item) => <tr key={item.id}><td>{item.supplier_name || "Sin proveedor"}</td><td>{item.service || "Servicio"}</td><td>{money(item.expected_amount ?? item.amount)}</td><td>{item.status || "expected"}</td></tr>)}</tbody></table></div> : <p>No hay compras esperadas.</p>}</section> : restricted("Esta sección no está disponible para tu rol.") : null}
    {tab === "actividad" && loading !== tab ? <ActivityTab caseId={props.initialCase.id} initialTasks={tasks} timeline={timeline} /> : null}
  </div>;
}
