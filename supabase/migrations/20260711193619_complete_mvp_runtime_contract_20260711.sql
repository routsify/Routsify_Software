-- Routsify MVP runtime contract: align deployed Supabase schema with the reviewed GitHub application.
...
create unique index if not exists bookings_event_idempotency_idx
  on public.bookings(organization_id, source, external_booking_id, event_type, event_timestamp);
...
create table if not exists public.webhook_events (
  ...
  channel text not null, event_id text not null, event_type text not null, payload_hash text,
  received_at timestamptz not null default now(), processed_at timestamptz,
  status text not null default 'received', last_error text,
...
declare tbl text;
begin
  foreach tbl in array array['leads','bookings','suppliers','supplier_invoices','travelers','contracts','billing_documents','tasks','timeline_events','margin_rules','proposal_acceptances','webhook_events']
  loop
