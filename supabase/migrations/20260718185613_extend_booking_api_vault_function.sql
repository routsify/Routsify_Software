create or replace function public.set_organization_secret(target_org uuid, target_key text, secret_value text, actor uuid default null)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_secret_id uuid;
begin
  if target_key not in ('holded_api_key','openai_api_key','fillout_webhook_secret','booking_webhook_secret','booking_api_key','smtp_username','smtp_password','whatsapp_access_token','whatsapp_verify_token','whatsapp_app_secret') then
  ...
  insert into public.organization_secrets(organization_id, secret_key, vault_secret_id, updated_by, created_at, updated_at)
  values (target_org, target_key, v_secret_id, actor, now(), now())
  on conflict (organization_id, secret_key) do update set vault_secret_id=excluded.vault_secret_id,ciphertext=null,iv=null,auth_tag=null,updated_by=excluded.updated_by,last_tested_at=null,last_test_status=null,last_test_message=null,updated_at=now();
