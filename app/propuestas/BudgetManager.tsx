...
const emptyLine: LineDraft = { service_type_code: "custom", description_public: "", description_internal: "", supplier_name: "", destination_segment: "", start_date: "", end_date: "", cost_budget: "", margin_applied: "20", sale_price: "", creates_expected_purchase: true };
...
function asCase(input: unknown): CaseOption {
  ...
  return { id: String(row.id || ""), case_code: String(row.case_code || "Expediente"), title: row.title ? String(row.title) : null, destination: row.destination ? String(row.destination) : null, clients: rawClients && typeof rawClients === "object" ? rawClients as CaseOption["clients"] : null };
}
...
function money(value: unknown) { return new Intl.NumberFormat("es-ES", { style: "currency", currency: "EUR" }).format(num(value)); }
...
function salePreview(cost: string, margin: string, explicitSale = "") { if (explicitSale.trim()) return num(explicitSale); const numericCost = num(cost); const numericMargin = Math.min(Math.max(num(margin), 0), 95); return numericCost > 0 ? numericCost / (1 - numericMargin / 100) : 0; }
function percentFromStored(value: unknown) { const number = num(value); return String(number <= 1 ? number * 100 : number); }
function draftFromLine(line: Line): LineDraft { return { service_type_code: line.service_type_code || "custom", description_public: line.description_public || "", description_internal: line.description_internal || "", supplier_name: line.supplier_name || "", destination_segment: line.destination_segment || "", start_date: line.start_date || "", end_date: line.end_date || "", cost_budget: String(line.cost_budget ?? ""), margin_applied: percentFromStored(line.margin_applied), sale_price: String(line.sale_price ?? ""), creates_expected_purchase: line.creates_expected_purchase !== false }; }
...
export function BudgetManager({ initialProposals = [], initialCases = [], initialCaseId = "" }: { initialProposals?: unknown[]; initialCases?: unknown[]; initialCaseId?: string }) {
  ...
  const preview = salePreview(line.cost_budget, line.margin_applied, line.sale_price);
  ...
  async function submitLine(event: FormEvent<HTMLFormElement>) {
    ...
    if (cost < 0 || margin < 0 || margin >= 100) return setMessage("Revisa el coste y el margen.");
    ...
    const payload = { ...line, description_public: description, proposal_version_id: selectedVersion.id, sale_price: line.sale_price.trim() || null };
    setSaving(true); setMessage(null);
  ...
  async function importAll() {
    ...
    replaceVersion(selected.id, selectedVersion.id, (version) => ({ ...version, budget_lines: [...(version.budget_lines || []), ...(result.data as Line[])] })); setImportText(""); setImportRows([]); setImportErrors([]); setTab("manual"); setMessage(`${result.imported} líneas importadas correctamente.`);
  }
  ...
  async function prepareShare() {
    ...
    const response = await fetch(`/api/routsify/proposals/${selected.id}/send`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ validity_days: 15 }) }); const result = await response.json().catch(() => null); setSaving(false);
    if (!response.ok || !result?.ok) return setMessage(String(result?.error || "No se pudo generar el enlace."));
    setShareData(result.data); setProposals((current) => current.map((proposal) => proposal.id === selected.id ? { ...proposal, status: "sent", proposal_versions: (proposal.proposal_versions || []).map((version) => version.id === latest?.id ? { ...version, status: "sent", expires_at: result.data.expires_at } : version) } : proposal)); setMessage("Enlace privado preparado.");
  }
  ...
  return <div className="budget-page">
    ...
    <section className="card budget-selector"><div className="client-filters client-filters-simple"><input className="input" placeholder="Buscar por expediente, cliente o destino..." value={query} onChange={(event) => setQuery(event.target.value)} /><button className={showCreate ? "btn secondary" : "btn"} type="button" onClick={() => setShowCreate((current) => !current)}>{showCreate ? "Cerrar formulario" : "Nuevo presupuesto"}</button></div>
      ...
      {proposals.length === 0 ? <div className="empty-state"><h2>Todavía no hay presupuestos</h2><p>Crea un expediente y después su presupuesto.</p></div> : <div className="table-scroll"><table><thead><tr><th>Expediente</th><th>Cliente</th><th>Estado</th><th>Versión</th><th>Líneas</th><th>Venta</th><th></th></tr></thead><tbody>{filtered.map((proposal) => <tr key={proposal.id} className={proposal.id === selected?.id ? "selected-row" : ""}><td><strong>{proposal.cases?.case_code || "Presupuesto"}</strong></td><td>{proposal.cases?.clients?.display_name || "—"}</td><td>{statusText(proposal.status)}</td><td>v{latestVersion(proposal)?.version_number || 1}</td><td>{linesOfVersion(latestVersion(proposal)).length}</td><td>{money(totals(proposal).sale)}</td><td><button className="btn secondary" type="button" onClick={() => selectProposal(proposal.id)}>Abrir</button></td></tr>)}</tbody></table></div>}
    </section>

    {selected ? <section className="card budget-workspace"><div className="budget-workspace-header"><div><span className="eyebrow">{selected.cases?.case_code}</span><h2>{selected.cases?.clients?.display_name || "Presupuesto"} · {selected.cases?.destination || "Sin destino"}</h2><p>Versión {selectedVersion?.version_number || 1} · {statusText(selected.status)} · {selectedLines.length} servicios</p></div><div className="budget-header-actions"><a className="btn secondary" href={`/expedientes?caseId=${encodeURIComponent(selected.cases?.id || "")}`}>Abrir expediente</a><a className="btn secondary" href={`/compras?caseId=${encodeURIComponent(selected.cases?.id || "")}`}>Compras</a></div></div>
      <div className="budget-totals"><div><span>Coste</span><strong>{money(selectedTotals.cost)}</strong></div><div><span>Venta</span><strong>{money(selectedTotals.sale)}</strong></div><div><span>Beneficio</span><strong>{money(selectedTotals.profit)}</strong></div><div><span>Margen</span><strong>{selectedTotals.sale ? `${selectedTotals.margin.toFixed(1)}%` : "0%"}</strong></div></div>
