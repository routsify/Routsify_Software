...
type UserOption = { user_id: string; full_name?: string | null; role?: string | null };
...
export function AutomationManager({ initialRules, initialExecutions, users }: { initialRules: unknown[]; initialExecutions: unknown[]; users: unknown[] }) {
  ...
  const userOptions = users.map((value) => { const row = object(value); return { user_id: String(row.user_id || ""), full_name: row.full_name ? String(row.full_name) : null, role: row.role ? String(row.role) : null } satisfies UserOption; }).filter((user) => user.user_id);
