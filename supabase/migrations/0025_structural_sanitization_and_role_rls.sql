...
-- One operational proposal root per case. Versioning belongs in proposal_versions.
...
-- Helper comments: API authorization remains the first layer; RLS mirrors the same role boundaries.
...
USING (organization_id = (SELECT public.current_org_id()));
CREATE POLICY leads_write_scoped ON public.leads FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]));
...
USING (organization_id = (SELECT public.current_org_id()));
CREATE POLICY bookings_write_scoped ON public.bookings FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]));
...
USING (organization_id = (SELECT public.current_org_id()));
CREATE POLICY cases_insert_scoped ON public.cases FOR INSERT TO authenticated
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]));
CREATE POLICY cases_update_scoped ON public.cases FOR UPDATE TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]));
CREATE POLICY cases_delete_scoped ON public.cases FOR DELETE TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction']::public.app_role[]));

-- Proposal roots, versions and lines
DROP POLICY IF EXISTS proposals_org_access ON public.proposals;
CREATE POLICY proposals_select_scoped ON public.proposals FOR SELECT TO authenticated
USING (organization_id = (SELECT public.current_org_id()));
CREATE POLICY proposals_write_scoped ON public.proposals FOR ALL TO authenticated
...
CREATE POLICY proposal_versions_select_scoped ON public.proposal_versions FOR SELECT TO authenticated
USING (organization_id = (SELECT public.current_org_id()));
CREATE POLICY proposal_versions_write_scoped ON public.proposal_versions FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales']::public.app_role[]));

DROP POLICY IF EXISTS budget_lines_org_access ON public.budget_lines;
CREATE POLICY budget_lines_select_scoped ON public.budget_lines FOR SELECT TO authenticated
USING (organization_id = (SELECT public.current_org_id()));
CREATE POLICY budget_lines_write_scoped ON public.budget_lines FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales']::public.app_role[]));

-- Supplier operations
DROP POLICY IF EXISTS expected_purchases_org_access ON public.expected_purchases;
CREATE POLICY expected_purchases_select_scoped ON public.expected_purchases FOR SELECT TO authenticated
...
CREATE POLICY expected_purchases_write_scoped ON public.expected_purchases FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations','billing']::public.app_role[]))
...
CREATE POLICY supplier_invoices_select_scoped ON public.supplier_invoices FOR SELECT TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations','billing']::public.app_role[]));
CREATE POLICY supplier_invoices_write_scoped ON public.supplier_invoices FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations','billing']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations','billing']::public.app_role[]));
...
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations','billing']::public.app_role[]));
CREATE POLICY contracts_write_scoped ON public.contracts FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations','billing']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations','billing']::public.app_role[]));
...
CREATE POLICY payments_write_scoped ON public.payments FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','billing']::public.app_role[]))
...
CREATE POLICY payment_links_select_scoped ON public.payment_links FOR SELECT TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales']::public.app_role[]));
CREATE POLICY payment_links_write_scoped ON public.payment_links FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales']::public.app_role[]));

DROP POLICY IF EXISTS billing_documents_org_access ON public.billing_documents;
CREATE POLICY billing_documents_select_scoped ON public.billing_documents FOR SELECT TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','billing']::public.app_role[]));
CREATE POLICY billing_documents_write_scoped ON public.billing_documents FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','billing']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','billing']::public.app_role[]));

-- Travelers
DROP POLICY IF EXISTS travelers_org_access ON public.travelers;
CREATE POLICY travelers_select_scoped ON public.travelers FOR SELECT TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]));
CREATE POLICY travelers_write_scoped ON public.travelers FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations']::public.app_role[]));
...
CREATE POLICY timeline_events_select_scoped ON public.timeline_events FOR SELECT TO authenticated
USING (organization_id = (SELECT public.current_org_id()));
CREATE POLICY timeline_events_write_scoped ON public.timeline_events FOR ALL TO authenticated
USING (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations','billing']::public.app_role[]))
WITH CHECK (organization_id = (SELECT public.current_org_id()) AND (SELECT public.current_app_role()) = ANY (ARRAY['admin','direction','sales','operations','billing']::public.app_role[]));
