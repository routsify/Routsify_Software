...
begin
  if new.status='closed'::public.case_status and old.status is distinct from new.status then
    ...
    if not exists(select 1 from public.contracts where organization_id=new.organization_id and case_id=new.id and status='signed') then raise exception 'signed_contract_required'; end if;
