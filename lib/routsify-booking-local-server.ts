...
export async function persistRemoteBooking(input: {
  ...
  const row = {
    ...
    starts_at: input.requestedStartsAt || existing?.starts_at || input.remote.startsAt || null,
    ends_at: input.requestedEndsAt || existing?.ends_at || input.remote.endsAt || null,
