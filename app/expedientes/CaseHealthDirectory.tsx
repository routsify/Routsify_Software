...
import type { CaseDirectoryPage, CaseDirectoryRow, CaseDirectoryStatus, CaseHealthLevel } from "@/lib/case-directory-server";
    ...
    <section className="clients-layout">
      <div className="card clients-main">
        <form className="client-filters client-filters-wide" onSubmit={submit}>
          ...
          <label>Estado<select value={status} onChange={(event) => { const next = event.target.value as CaseDirectoryStatus; setStatus(next); void load(1, pageSize, query, next, health); }} disabled={loading}><option value="active">Activos</option><option value="all">Todos</option><option value="closed">Cerrados</option>{activeStatusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
