...
export function DocumentsTab({ caseId, caseCode, initialDocuments, travelers, onChange, onTravelerChange }: { caseId: string; caseCode: string; initialDocuments: DocumentRow[]; travelers: Traveler[]; onChange?: (documents: DocumentRow[]) => void; onTravelerChange?: (traveler: Traveler) => void }) {
  ...
  async function uploadDocument(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setItems((current) => { const next = [confirm.data, ...current]; onChange?.(next); return next; }); form.reset(); setMessage("Documento subido correctamente. Se conservará durante cinco años.");
  ...
  async function processOcr(documentId: string) {
    ...
    setItems((current) => { const next = current.map((item) => item.id === documentId ? { ...item, ocr_status: "review_required" } : item); onChange?.(next); return next; });
  ...
  async function approveOcr(event: FormEvent<HTMLFormElement>) {
    ...
    setItems((current) => { const next = current.map((item) => item.id === review.documentId ? { ...item, ocr_status: "approved" } : item); onChange?.(next); return next; });
    if (result.data?.traveler?.id) onTravelerChange?.(result.data.traveler as Traveler);
