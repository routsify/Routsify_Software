...
const definitions = {
  holded_api_key: { title: "Holded", description: "Sincroniza contactos, presupuestos, proformas, facturas, compras y pagos.", integration: "holded", placeholder: "Introduce la credencial de Holded" },
  openai_api_key: { title: "OpenAI OCR", description: "Extrae datos de DNI y pasaportes con revisión humana obligatoria.", integration: "openai", placeholder: "Introduce la credencial de OpenAI" },
} as const;
...
export function IntegrationSecretsPanel({ initialStatuses, canManage }: { initialStatuses: SecretStatus[]; canManage: boolean }) {
  ...
  return <section className="card">
    <div className="panel-head"><div><h2>Claves de integraciones</h2><p>Las claves se guardan cifradas y separadas por organización. El navegador nunca recibe la clave guardada.</p></div><span className="badge">Solo administrador</span></div>
      ...
      {(Object.keys(definitions) as Array<keyof typeof definitions>).map((key) => {
        ...
        return <article className="card" key={key}>
          ...
          <form className="form" onSubmit={(event) => void save(event, key)}>
            <label>Nueva credencial<input className="input" type="password" autoComplete="new-password" placeholder={definition.placeholder} value={values[key]} onChange={(event) => setValues((current) => ({ ...current, [key]: event.target.value }))} disabled={!canManage || busy !== null} /></label>
