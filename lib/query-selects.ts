...
export const CASE_WORKSPACE_PROPOSALS_SELECT = [
  "id,organization_id,case_id,status,current_version_id,created_at,updated_at",
  "proposal_versions:proposal_versions!proposal_versions_proposal_id_fkey(id,proposal_id,version_number,status,locked,total_sale,total_cost,total_cost_budget,budgeted_profit,created_at,expires_at,budget_lines(id,proposal_version_id,stable_line_id,service_type_code,description_public,description_internal,supplier_id,supplier_name,destination_segment,start_date,end_date,cost_budget,margin_applied,sale_price,creates_expected_purchase,sort_order,created_at))",
].join(",");
