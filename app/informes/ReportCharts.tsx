...
export const chartPad = { padding: "0 18px 18px" };
export const cardStyle = { padding: 0, overflow: "hidden" };

export function ReportCard({ title, action, children }: { title: string; action?: React.ReactNode; children: React.ReactNode }) {
  return <section className="card report-card" style={cardStyle}><div className="panel-head"><h2>{title}</h2>{action}</div><div style={chartPad}>{children}</div></section>;
}
...
export function ValueLineChart({ data = timeSeries() }: { data?: ReturnType<typeof timeSeries> }) {
  ...
  return <svg viewBox="0 0 420 250" role="img" aria-label="Evolución del valor aceptado" style={{ width: "100%", minHeight: 250 }}>
...
export function FunnelVisual({ data = funnelSteps() }: { data?: ReturnType<typeof funnelSteps> }) {
  ...
  return <div style={{ display: "grid", gap: 8 }}>{data.map((step, index) => <a key={step.key} href={step.url} style={{ display: "grid", gridTemplateColumns: "170px 1fr 54px 58px", gap: 10, alignItems: "center" }}><strong style={{ fontSize: 12 }}>{step.label}</strong><span style={{ height: 26, borderRadius: 8, background: "#eef5f6", overflow: "hidden" }}><span style={{ display: "block", width: `${Math.max(8, (step.count / max) * 100)}%`, height: "100%", borderRadius: 8, background: colors[index % colors.length], opacity: 0.72 }} /></span><b>{step.count}</b><small>{percent(step.conversionFromLeadPct)}</small></a>)}</div>;
}
...
export function DestinationDonut({ data = destinations() }: { data?: ReturnType<typeof destinations> }) {
  ...
  return <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 18, alignItems: "center" }}><svg viewBox="0 0 180 180" style={{ width: "100%", maxWidth: 220 }}>
    <circle cx="90" cy="90" r={radius} fill="none" stroke="#eef5f6" strokeWidth="28" />
    {data.map((item, index) => { const dash = (item.value / total) * circumference; const offset = -current; current += dash; return <circle key={item.destination} cx="90" cy="90" r={radius} fill="none" stroke={colors[index % colors.length]} strokeWidth="28" strokeDasharray={`${dash} ${circumference - dash}`} strokeDashoffset={offset} transform="rotate(-90 90 90)" />; })}
    <text x="90" y="88" textAnchor="middle" fontSize="18" fontWeight="900" fill="#102f3c">{money(total)}</text><text x="90" y="108" textAnchor="middle" fontSize="11" fill="#6c7f89">Total</text>
  </svg><div className="donut-list">{data.map((item) => <a key={item.destination} href={`/expedientes?destination=${item.destination}`}><strong>{item.destination}</strong><span>{money(item.value)} · {percent(item.sharePct)}</span></a>)}</div></div>;
}
...
export function FinanceLines({ data = timeSeries() }: { data?: ReturnType<typeof timeSeries> }) {
  ...
  return <svg viewBox="0 0 420 250" role="img" aria-label="Ingresos costes beneficio" style={{ width: "100%", minHeight: 250 }}>
