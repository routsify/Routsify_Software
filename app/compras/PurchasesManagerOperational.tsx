...
type BudgetLine = { id?: string; service_type_code?: string | null; description_public?: string | null; description_internal?: string | null; destination_segment?: string | null; start_date?: string | null; end_date?: string | null; cost_budget?: number | string | null; sale_price?: number | string | null };
type Purchase = {
  id: string;
  case_id?: string | null;
  proposal_version_id?: string | null;
  budget_line_id?: string | null;
  cases?: { id?: string | null; case_code?: string | null; title?: string | null } | null;
...
function normalizeCase(input: unknown): CaseOption { const row = input as Record<string, unknown>; const rawClient = Array.isArray(row.clients) ? row.clients[0] : row.clients; return { id: String(row.id || ""), case_code: String(row.case_code || "Expediente"), title: row.title ? String(row.title) : null, clients: rawClient && typeof rawClient === "object" ? rawClient as CaseOption["clients"] : null }; }
...
export function PurchasesManagerOperational({ initialPurchases = [], initialCases = [], initialCaseId = "" }: { initialPurchases?: unknown[]; initialCases?: unknown[]; initialCaseId?: string }) {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(() => normalizedItems.find((item) => item.case_id === initialCaseId)?.id || normalizedItems[0]?.id || null);
  ...
  const filtered = useMemo(() => items.filter((item) => {
    const text = [item.supplier_name, item.service, item.cases?.case_code, item.budget_lines?.description_public, item.budget_lines?.destination_segment].filter(Boolean).join(" ").toLowerCase();
  ...
  function replaceItem(input: unknown) { const updated = normalize(input); setItems((current) => current.map((item) => item.id === updated.id ? updated : item)); setSelectedId(updated.id); }
  ...
  async function uploadInvoice(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const confirmResponse = await fetch("/api/documentos/confirm-upload", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ownerType: "supplier_invoice", ownerId: selected.id, caseId: selected.case_id, bucket: signed.bucket, storagePath: signed.path, fileName: invoiceFile.name, mimeType: invoiceFile.type, sizeBytes: invoiceFile.size, title: `Factura ${selected.supplier_name || "proveedor"}`, invoiceNumber: invoiceDraft.invoiceNumber || null, invoiceDate: invoiceDraft.invoiceDate || null, invoiceBase: optionalNumber(invoiceDraft.invoiceBase), invoiceTax: optionalNumber(invoiceDraft.invoiceTax), invoiceTotal: optionalNumber(invoiceDraft.invoiceTotal), currency: invoiceDraft.currency || "EUR", retentionDays: 365 }) });
      ...
      await refreshPurchase(selected.id); setInvoiceDraft(emptyInvoice); setInvoiceFile(null); setMessage("Factura subida y vinculada. Queda pendiente de revisión.");
  ...
  return <div className="clients-page">
    <section className="client-kpis"><div className="kpi-card"><span className="kpi-icon">C</span><span className="kpi-copy"><strong>Compras</strong><b>{items.length}</b><small>Total registradas</small></span></div><div className="kpi-card"><span className="kpi-icon">P</span><span className="kpi-copy"><strong>Pendientes</strong><b>{pending}</b><small>Por resolver</small></span></div><div className="kpi-card"><span className="kpi-icon">R</span><span className="kpi-copy"><strong>Revisión</strong><b>{review}</b><small>Necesitan decisión</small></span></div><div className="kpi-card"><span className="kpi-icon">€</span><span className="kpi-copy"><strong>Coste previsto</strong><b>{money(total)}</b><small>De todos los expedientes</small></span></div></section>
    <section className="clients-layout"><div className="card clients-main"><div className="client-filters client-filters-wide"><input className="input" placeholder="Buscar proveedor, servicio, línea o expediente..." value={query} onChange={(event) => setQuery(event.target.value)} /><label>Estado<select value={filter} onChange={(event) => setFilter(event.target.value)}><option>Todos</option>{statusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button className={showCreate ? "btn secondary" : "btn"} type="button" onClick={() => setShowCreate((value) => !value)}>{showCreate ? "Cerrar formulario" : "Nueva compra manual"}</button></div>
      ...
      <section className="side-section"><h3>Revisión</h3>{["uploaded", "matched", "review_needed"].includes(String(selected.status)) ? <button className="btn" type="button" disabled={savingId === selected.id} onClick={() => void updateStatus(selected.id, "approved")}>Aprobar factura</button> : null}<button className="btn secondary" type="button" disabled={savingId === selected.id} onClick={() => void updateStatus(selected.id, "requested")}>Marcar como solicitada</button><label>Motivo para no requerir factura<textarea className="input" rows={2} value={notRequiredReason} onChange={(event) => setNotRequiredReason(event.target.value)} /></label><button className="btn secondary" type="button" disabled={savingId === selected.id || notRequiredReason.trim().length < 5} onClick={() => void updateStatus(selected.id, "not_required", { reason: notRequiredReason })}>Marcar no necesaria</button></section></> : <div className="empty-state"><h2>Sin compra seleccionada</h2></div>}</aside></section>
  </div>;
}
