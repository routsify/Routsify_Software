...
export function PurchasesManagerOperational({ initialPurchases = [], initialCases = [], initialSuppliers = [], initialCaseId = "", initialSupplierId = "" }: { initialPurchases?: unknown[]; initialCases?: unknown[]; initialSuppliers?: unknown[]; initialCaseId?: string; initialSupplierId?: string }) {
  ...
  const [draft, setDraft] = useState<Draft>(() => ({ ...emptyDraft, case_id: cases.some((item) => item.id === initialCaseId) ? initialCaseId : "", supplier_id: suppliers.some((item) => item.id === initialSupplierId && item.active !== false) ? initialSupplierId : "" }));
  ...
  const filtered = useMemo(() => items.filter((item) => {
    const text = [item.suppliers?.name, item.supplier_name, item.service, item.cases?.case_code, item.budget_lines?.description_public, item.budget_lines?.destination_segment].filter(Boolean).join(" ").toLowerCase();
  ...
  const forecastTotal = items.reduce((sum, item) => sum + expectedCost(item), 0);
  const approvedTotal = items.reduce((sum, item) => sum + realCost(item), 0);
  ...
  async function createPurchase(event: FormEvent<HTMLFormElement>) {
    ...
    const created = normalize(result.data); setItems((current) => [created, ...current]); selectPurchase(created); setDraft(emptyDraft); setShowCreate(false); setMessage("Compra creada y vinculada al proveedor maestro.");
  }
  ...
  async function uploadInvoice(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await refreshPurchase(selected.id); setInvoiceDraft(emptyInvoice); setInvoiceFile(null); setMessage("Factura subida y vinculada. Revisa el coste real antes de aprobarla.");
