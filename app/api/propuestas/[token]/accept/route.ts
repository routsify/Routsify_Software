...
export async function POST(_request: NextRequest, context: { params: Promise<{ token: string }> }) {
  ...
  if (!hasSupabaseAdminEnv() || resolved.mode === "demo") return NextResponse.json({ ok: true, mode: resolved.mode, proposalId: resolved.proposalId, versionId: resolved.versionId });
