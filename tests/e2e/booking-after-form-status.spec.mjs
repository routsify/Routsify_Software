...
test("Booking creates updates and cancels with Fillout status", async ({ page }) => {
  ...
  try {
    ...
    await call(page, `/api/routsify/clients/booking/reservations/${localId}`, "PATCH", {
      notes: "[PRUEBA E2E ROUTSIFY] actualización verificada",
    });
  } finally {
    if (localId) {
  ...
  console.log("BOOKING_CERTIFIED", JSON.stringify({ localId, externalId, status, initialFormCompleted }));
});
