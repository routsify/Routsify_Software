...
export function PurchasesManagerOperational({ initialPurchases = [], initialCases = [], initialSuppliers = [], initialCaseId = "", initialSupplierId = "" }: { initialPurchases?: unknown[]; initialCases?: unknown[]; initialSuppliers?: unknown[]; initialCaseId?: string; initialSupplierId?: string }) {
  ...
  return <div className="clients-page">
    ...
    <section className="clients-layout"><div className="card clients-main"><div className="client-filters client-filters-wide"><input className="input" placeholder="Buscar proveedor, servicio, línea, expediente o moneda..." value={query} onChange={(event) => setQuery(event.target.value)} /><label>Estado<select value={filter} onChange={(event) => setFilter(event.target.value)}><option>Todos</option>{statusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Proveedor<select value={supplierFilter} onChange={(event) => setSupplierFilter(event.target.value)}><option value="Todos">Todos</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></label><button className={showCreate ? "btn secondary" : "btn"} type="button" onClick={() => setShowCreate((value) => !value)}>{showCreate ? "Cerrar formulario" : "Nueva compra manual"}</button></div>
      ...
      {filtered.length ? <div className="table-scroll"><table><thead><tr><th>Expediente</th><th>Proveedor</th><th>Servicio / línea</th><th>Previsto</th><th>Real</th><th>Desviación</th><th>Estado</th><th><span className="sr-only">Acciones</span></th></tr></thead><tbody>{filtered.map((item) => { const expected = expectedCost(item); const real = realCost(item); const deviation = real ? real - expected : 0; return <tr key={item.id} className={item.id === selected?.id ? "selected-row" : ""}><td>{item.cases?.case_code || "—"}<br /><small>{item.currency || item.cases?.currency || "EUR"}</small></td><td><button className="table-link" type="button" onClick={() => selectPurchase(item)}><strong>{item.suppliers?.name || item.supplier_name || "Proveedor"}</strong></button></td><td>{item.service || item.budget_lines?.description_public || "—"}<br /><small>{item.budget_line_id ? "Generada desde presupuesto" : "Compra manual"}</small></td><td>{money(expected, item.currency || "EUR")}</td><td>{real ? money(real, item.currency || "EUR") : "Pendiente"}</td><td>{real ? money(deviation, item.currency || "EUR") : "—"}</td><td>{statusLabel(item.status)}</td><td><RowActionMenu label={`Acciones para ${item.service || item.suppliers?.name || item.supplier_name || "compra"}`}><button type="button" onClick={() => selectPurchase(item)}>Abrir compra</button>{item.cases?.case_code ? <a href={`/expedientes/${encodeURIComponent(item.cases.case_code)}`}>Abrir expediente</a> : null}{item.supplier_id ? <a href={`/proveedores/${encodeURIComponent(item.supplier_id)}`}>Abrir proveedor</a> : null}{canManage ? <button className="danger-text" type="button" disabled={savingId === item.id} onClick={() => void deletePurchase(item)}>{savingId === item.id ? "Eliminando…" : "Eliminar"}</button> : null}</RowActionMenu></td></tr>; })}</tbody></table></div> : <div className="empty-state"><h2>Sin compras</h2><p>Las compras esperadas aparecerán al aceptar una propuesta con líneas marcadas para proveedor.</p></div>}
