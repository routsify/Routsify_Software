...
const emptyLine: LineDraft = { service_type_code: "custom", description_public: "", description_internal: "", supplier_id: "", supplier_name: "", destination_segment: "", start_date: "", end_date: "", cost_budget: "", margin_applied: "20", sale_price: "", creates_expected_purchase: true };
...
function asSupplier(input: unknown): Supplier { const row = input as Record<string, unknown>; return { id: String(row.id || ""), name: String(row.name || "Proveedor"), category: row.category ? String(row.category) : null, active: row.active !== false }; }
...
export function BudgetManager({ initialProposals = [], initialCases = [], initialSuppliers = [], initialCaseId = "" }: { initialProposals?: unknown[]; initialCases?: unknown[]; initialSuppliers?: unknown[]; initialCaseId?: string }) {
  ...
  const [suppliers] = useState<Supplier[]>(() => initialSuppliers.map(asSupplier).filter((item) => item.id));
  ...
  async function submitLine(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { ...line, supplier_id: supplier?.id || null, supplier_name: supplier?.name || null, description_public: description, proposal_version_id: selectedVersion.id, sale_price: line.sale_price.trim() || null };
    setSaving(true); setMessage(null);
  ...
  return <div className="budget-page">
    ...
    {selected ? <section className="card budget-workspace"><div className="budget-workspace-header"><div><span className="eyebrow">{selected.cases?.case_code}</span><h2>{selected.cases?.clients?.display_name || "Presupuesto"} · {selected.cases?.destination || "Sin destino"}</h2><p>Versión {selectedVersion?.version_number || 1} · {statusText(selected.status)} · {selectedLines.length} servicios</p></div><div className="budget-header-actions"><a className="btn secondary" href={`/expedientes?caseId=${encodeURIComponent(selected.cases?.id || "")}`}>Abrir expediente</a><a className="btn secondary" href={`/compras?caseId=${encodeURIComponent(selected.cases?.id || "")}`}>Compras</a><a className="btn secondary" href="/proveedores">Proveedores</a></div></div>
      <div className="budget-totals"><div><span>Coste presupuestado</span><strong>{money(selectedTotals.cost)}</strong></div><div><span>Venta</span><strong>{money(selectedTotals.sale)}</strong></div><div><span>Beneficio previsto</span><strong>{money(selectedTotals.profit)}</strong></div><div><span>Margen previsto</span><strong>{selectedTotals.sale ? `${selectedTotals.margin.toFixed(1)}%` : "0%"}</strong></div><div><span>Coste real / estimado</span><strong>{money(selectedTotals.realCost)}</strong></div><div><span>Beneficio real / estimado</span><strong>{money(selectedTotals.realProfit)}</strong><small>{selectedTotals.hasRealCost ? ` · margen ${selectedTotals.realMargin.toFixed(1)}% · desviación ${money(selectedTotals.deviation)}` : " · pendiente de compras reales"}</small></div></div>
