...
import { importClientsFromCsv } from "@/lib/client-import-server";
...
export async function POST(request: NextRequest) {
  ...
  if (!fileName.endsWith(".csv")) return NextResponse.json({ ok: false, error: "import_csv_required" }, { status: 400 });

  try {
    const data = await importClientsFromCsv(organizationId, await file.text());
    return NextResponse.json({ ok: true, data });
  } catch (error) {
    const message = error instanceof Error ? error.message : "client_import_failed";
